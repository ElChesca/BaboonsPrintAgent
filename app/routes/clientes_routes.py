...
def create_cliente(current_user, negocio_id):
    ...
    try:
        db.execute(
            'INSERT INTO clientes (nombre, dni, telefono, email, negocio_id) VALUES (%s, %s, %s, %s, %s) RETURNING id',
            (data['nombre'], data.get('dni'), data.get('telefono'), data.get('email'), negocio_id)
        )
