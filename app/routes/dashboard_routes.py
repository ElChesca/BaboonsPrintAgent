...
def get_dashboard_stats(current_user, negocio_id):
    db = get_db()
    db.execute("SELECT SUM(total) as total FROM ventas WHERE negocio_id = %s AND fecha::date = CURRENT_DATE", (negocio_id,))
    ventas_row = db.fetchone()
    ventas_hoy = ventas_row['total'] if ventas_row and ventas_row['total'] is not None else 0

    db.execute("SELECT COUNT(*) as count FROM productos WHERE negocio_id = %s AND stock <= stock_minimo AND stock > 0", (negocio_id,))
