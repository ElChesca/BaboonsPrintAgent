...
# [GET] Obtener todos los comentarios de un reclamo
@bp.route('/consorcio/reclamos/<int:reclamo_id>/comentarios', methods=['GET'])
@token_required
def get_reclamo_comentarios(current_user, reclamo_id):
    db = get_db()
    
    # --- Verificación de Seguridad ---
    # 1. Obtener el reclamo y sus datos clave
    db.execute(
        """
        SELECT r.negocio_id, u.inquilino_id, u.propietario_id
        FROM consorcio_reclamos r
        JOIN consorcio_unidades u ON r.unidad_id = u.id
        WHERE r.id = %s
        """,
    ...
    # 2. Validar que el usuario pueda ver este reclamo
    es_admin = current_user['rol'] in ('admin', 'superadmin')
    es_miembro = current_user['id'] in (reclamo_info['inquilino_id'], reclamo_info['propietario_id'])
    
    if not es_admin and not es_miembro:
        return jsonify({'error': 'No tiene permiso para ver este reclamo'}), 403
        
    # 3. Si tiene permiso, obtener comentarios
    try:
        db.execute(
            """
            SELECT c.id, c.comentario, c.fecha_creacion, u.nombre AS usuario_nombre, u.rol AS usuario_rol
            FROM consorcio_reclamos_comentarios c
            JOIN usuarios u ON c.usuario_id = u.id
            WHERE c.reclamo_id = %s
        ...
        print(f"Error en get_reclamo_comentarios: {e}")
        return jsonify({'error': str(e)}), 500
