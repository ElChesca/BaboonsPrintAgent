...
function poblarSelectoresConDatos(negocios) {
    ...
    const fillSelector = (selector, selectorName) => {
        ...
        if (savedNegocioId && negocios.some(n => String(n.id) === String(savedNegocioId))) {
            idSeleccionado = savedNegocioId; // Usa el guardado si es válido
        } else {
            // Si no hay guardado o no es válido, usa el primero y actualiza el estado
    ...
    fillSelector(homeSelector, 'homeSelector'); // homeSelector puede ser null, fillSelector lo maneja
    
    // Asegura que appState.negocioActivoId tenga un valor si hay negocios
