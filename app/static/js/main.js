...
// 1.5.8: Renombrado de bot├│n "Deshacer visita" a "Deshacer bajada" para mayor claridad.
// 1.5.3: Cambio de texto a "CONFIRMAR BAJADA" en app choferes.
// 1.5.2: Correcci├│n bug visual de suma string ("0.00" + 2 = "0.002") en las cantidades originales.
window.APP_VERSION = APP_VERSION;
const v = `?v=${APP_VERSION}`;

// --- AUTO-LIMPIEZA DE CACH├ë LOCAL ---
window.chequearVersionApp = () => {
    const versionGuardada = localStorage.getItem('app_version');

    // Si la versi├│n del c├│digo es nueva
    if (versionGuardada !== APP_VERSION) {
        console.warn(`Nueva versi├│n(${APP_VERSION}).Limpiando cach├® local cr├¡tica...`);
        localStorage.setItem('app_version', APP_VERSION);

        // Forzamos recarga si ya hab├¡a entrado antes
        if (versionGuardada) {
...
// --- 1.5. SANEAMIENTO DE SEGURIDAD (CR├ìTICO) ---
// Si por alg├║n motivo el formulario se envi├│ por GET, limpiamos la URL de inmediato
...
(function sanearUrlSeguridad() {
    ...
    if (urlParams.has('password') || urlParams.has('email')) {
        console.error("ÔÜá´©Å [SEGURIDAD] Credenciales detectadas en la URL. Limpiando...");
        ...
        // Reemplazamos el historial para que no quede rastro de la contrase├▒a
        window.history.replaceState({}, '', newUrl);
        
        // Opcional: Mostrar aviso si el login.js ya carg├│
        setTimeout(() => {
...
// --- 2.5 EXPOSICI├ôN GLOBAL (CR├ìTICO PARA M├ôDULOS LEGACY) ---
...
// --- EXPOSICI├ôN DE FUNCIONES GLOBALES ---
...
export function toggleMenu() {
    ...
    if (navContainer) {
        ...
        console.error("No se encontr├│ '.nav-container' al hacer toggle.");
    }
}
...
export const appState = {
    ...
    userRol: null,
    userId: null, 
    userEspecialidad: null, // ✨ Especialidad (mozo, cocinero, etc)
    filtroProveedorId: null,
    permissions: {}, // 🛡️ Permisos dinámicos cargados del backend
...
export async function checkSubscriptionStatus() {
    ...
    try {
        // Usamos fetchData para que maneje el token autom├íticamente y sea m├ís limpio
        ...
        if (data.status === 'blocked') {
            ...
            // Bloqueo total, el checkSubscriptionStatus se llama despu├®s de fetchAppPermissions
        ...
        console.warn("No se pudo verificar el estado de suscripci├│n:", error.message);
    }
}

// Ô£¿ NUEVA FUNCI├ôN: Ocultar todo el ERP si est├í bloqueado
function aplicarBloqueoPorMora(mensajeLocal) {
    if (appState.userRol === 'superadmin') {
        ...
        if (banner) {
            ...
            banner.style.color = 'white';
            banner.innerHTML = `<strong>ÔÜá´©Å ESTE NEGOCIO EST├ü BLOQUEADO POR MORA.</strong> (T├║ puedes verlo por ser Superadmin)`;
        }
        return;
    }
    ...
    if (contentArea) {
        contentArea.innerHTML = `
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 70vh; text-align: center; font-family: sans-serif; padding: 20px;">
                <h1 style="color: #dc3545; font-size: 3rem; margin-bottom: 20px;">ÔÜá´©Å Acceso Suspendido</h1>
                ...
                <p style="font-size: 1.2rem; color: #666; margin-top: 20px;">
                    Por favor, comun├¡quese con la administraci├│n para regularizar su situaci├│n.
                </p>
            </div>
        `;
    }
...
function loadPageCSS(pageName) {
    ...
    if (pageName) {
        ...
        if (pageName === 'login') cssPath = 'login_secure'; // Ô£¿ Evitar 404 si el hash es #login
        ...
        link.onerror = () => {
            // console.warn(`Advertencia: No se encontr├│ CSS opcional en ${ link.href } `);
            link.remove();
        };
    }
}
...
function generarNavbarDinamico() {
    ...
    // 1. Agrupar m├│dulos por categor├¡a
    const categories = {
        'operaciones': { label: 'Operaciones', items: [] },
        'administrativo': { label: 'Administración', items: [] },
        'reportes': { label: 'Reportes', items: [] },
        'reglas': { label: 'Configuración', items: [] }
    };
    ...
    // 3. Agregar Links Especiales (siempre visibles o con lógica extra)
    if (isSuper) {
...
function actualizarVisibilidadMenu() {
    // Ahora actualizarVisibilidadMenu genera el menú desde cero
    generarNavbarDinamico();
    
    // Fallback para filtros legacy si quedara algún link hardcodeado en index.html
...
/**
 * 🎨 RENDERIZA DINÁMICAMENTE EL DASHBOARD (TARJETAS)
 * Oculta las tarjetas que apuntan a módulos sin permiso y construye las necesarias.
 */
function filtrarTarjetasDashboards() {
    ...
    console.log(`🎨 [Dashboard] Renderizando para ${tipoAppActual}. Permisos:`, perms);
...
// Ô£¿ NUEVA FUNCI├ôN: Verificar estado de caja GLOBALMENTE (para validaciones en otros m├│dulos)
export async function checkGlobalCashRegisterState() {
    ...
    try {
        ...
        if (data.estado === 'abierta' && data.sesion) {
            ...
            // console.log(`Ô£à Caja Abierta detectada.Sesi├│n ID: ${ appState.cajaSesionIdActiva } `);
        } else {
            appState.cajaSesionIdActiva = null;
            // console.log("Ôä╣´©Å Caja Cerrada o sin sesi├│n activa.");
        }
    } catch (error) {
...
async function poblarSelectorNegocios() {
    ...
    try {
        ...
        if (!Array.isArray(negocios)) {
            console.error("fetchData('/api/negocios') no devolvi├│ un array:", negocios);
            throw new Error("Datos de negocios inv├ílidos.");
        }
        ...
        // Ô£¿ Verificar suscripci├│n al cambiar de negocio
        ...
        if (appState.negocioActivoId) {
            try {
                ...
                if (inactiveModules.length > 0) {
                    console.log(`­ƒÜ½ Aplicando exclusiones por negocio (${appState.negocioActivoId}):`, inactiveModules);
                ...
                console.warn("No se pudo cargar la configuraci├│n de m├│dulos por negocio.");
            }
        }
...
export async function actualizarUIAutenticacion() {
    ...
    try {
        ...
        if (user && user.nombre && user.rol) {
            // Ô£à USUARIO LOGUEADO: Limpiar estado de login
            document.body.classList.remove('login-page');
            document.body.style.overflow = 'auto'; // Restaurar scroll
            
            appState.userRol = user.rol;
            appState.userId = user.id; // ✨ Guardar ID de usuario
            appState.userEspecialidad = user.especialidad; // ✨ Capturar especialidad (mozo, etc)
            ...
            // Ô£¿ CARGAR NEGOCIOS PRIMERO (Para tener el ID activo)
            await poblarSelectorNegocios();

            // Ô£¿ VERIFICAR ESTADO DE CAJA (CR├ìTICO PARA VALIDACIONES)
            await checkGlobalCashRegisterState();

            // Ô£¿ LUEGO CARGAR PERMISOS (Que depende del ID activo)
            await fetchAppPermissions();

            // Ô£¿ CONFIGURAR TIMER DE INACTIVIDAD
            const { setupInactivityTimer } = await import(`./modules/auth.js${v}`);
            setupInactivityTimer();

            // Ô£¿ LIMPIEZA PREVENTIVA: Si estamos logueados, el login-page-wrapper DEBE morir.
            const contentArea = document.getElementById('content-area');
            ...
            actualizarVisibilidadMenu(); // Ô£¿ Filtrar navbar seg├║n permisos

            // Ô£¿ BLOQUEO POR MORA
            if (appState.subscriptionStatus === 'blocked' && appState.userRol !== 'superadmin') {
                return; // La funci├│n aplicarBloqueoPorMora ya se encarg├│ de dibujar la pantalla
            }

            const requestedPage = window.location.hash.substring(1).split('?')[0];

            // L├│gica de Home Din├ímico
            ...
            // Ô£¿ REDIRECCI├ôN FORZADA: Si est├í en una home de otro tipo de negocio, mandarlo a la suya
            ...
            // VALIDACI├ôN DE SEGURIDAD CON PERMISOS DIN├üMICOS
            ...
            if (tipoAppActual && pageToLoad !== 'login' && pageToLoad !== 'admin_apps' && appState.userRol !== 'chofer') {
                ...
                // Si es superadmin, acceso total a admin_apps, pero validamos m├│dulos de negocio igual
                if (!esHomeDelNegocio && !rutasPermitidas.includes(pageToLoad) && !rutasComunes.includes(pageToLoad)) {
                    ...
                    console.warn(`Acceso no autorizado: Usuario '${tipoAppActual}' intent├│ cargar '${pageToLoad}'.`);
                    // NO forzamos redirecci├│n a home aqu├¡ para evitar bucles si la home tambi├®n est├í bloqueada.
            ...
            if (requestedPage !== pageToLoad) {
                ...
                // Ô£¿ FIX: Si el hash ya cambi├│ pero popstate est├í bloqueado por estaActualizandoAuth,
                // forzamos la carga manual aqu├¡ para que el login desaparezca.
            ...
            // ­ƒöÆ SEGURIDAD: SI ESTAMOS EN EL LOGIN, CERRAR SESI├ôN SIEMPRE
            if (window.location.hash === '#login' || window.location.hash === '') {
                ...
                if (localStorage.getItem('jwt_token')) {
                    console.warn("Sesi├│n activa detectada en p├ígina de login. Limpiando para seguridad.");
            ...
            // Usamos un peque├▒o timeout para dar tiempo al navegador a procesar el cambio de hash
            setTimeout(() => {
                // Pasamos true en el 4to argumento para forzar la carga del HTML
                // aunque el hash ya sea #login
                // Ô£¿ CACHE BUSTER RENAMED: Agregamos timestamp para asegurar que cargue la versi├│n sin el link inseguro
    ...
    try {
        switch (pageName) {
            ...
            case 'rentals_dashboard':
                // Dashboards est├íticos, el filtrado de tarjetas se maneja en loadContent
                break;
            case 'home_chofer':
...
const LOOP_LIMIT = 5; // M├íximo de navegaciones permitidas
...
function checkLoopProtection() {
    ...
    if (history.length > LOOP_LIMIT) {
        console.error("­ƒöÑ BUCLE DETECTADO. DETENIENDO EJECUCI├ôN ­ƒöÑ");
        document.body.innerHTML = `
            <div style="padding:50px; text-align:center; color:#721c24; background:#f8d7da; font-family:sans-serif; height:100vh; display:flex; flex-direction:column; justify-content:center; align-items:center;">
                <h1 style="font-size: 3rem;">ÔÜá´©Å SISTEMA DETENIDO ÔÜá´©Å</h1>
                ...
                <button onclick="sessionStorage.clear(); window.location.href='/#login'; window.location.reload();" 
                    ...
                    ÔÖ╗´©Å REINICIAR SISTEMA (Borrar Cach├®)
                </button>
            </div>
        `;
...
/**
 * 🔍 REVERSO DE MAPPING (Ruta -> Key de Permiso)
 * Busca en el ERP_REGISTRY qué permiso corresponde a un archivo físico.
 */
function getRegistryKeyByPath(pageName) {
    ...
    return pageName; // Si no está en el registro, devolvemos el original
}

// --- loadContent ---
export function loadContent(event, page, clickedLink, fromHistory = false) {
    checkLoopProtection(); // <--- PROTECCI├ôN ACTIVADA
    ...
    return fetch(pageToFetch)
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    Swal.fire({
                        ...
                        text: `El archivo correspondiente a "${page}" no se encuentra disponible momentáneamente. Por favor, contacta a soporte.`,
                        confirmButtonColor: '#4f46e5'
                    });
                    throw new Error(`404: Modulo ${page} no encontrado físicamente.`);
                }
        ...
        .then(html => {
            ...
            if (contentArea) {
                ...
                requestAnimationFrame(() => {
                    ...
                    // Ô£¿ FILTRAR TARJETAS EN DASHBOARDS (Si aplica)
        ...
        .catch(error => {
            ...
            if (currentHash === defaultHomePage || pageName === defaultHomePage) {
                console.error("Fallo cr├¡tico en p├ígina de inicio. Deteniendo para evitar bucle.");
...
/**
 * Ô£¿ L├ôGICA DE MODAL DE NUEVO CLIENTE "AL VUELO"
...
export async function abrirModalNuevoCliente(callback) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 500px;">
                <h3 style="margin-bottom: 20px;">­ƒæñ Registro R├ípido de Cliente</h3>
                <form id="form-cliente-quick">
                    <div class="form-group">
                        <label>Nombre Completo:</label>
                        <input type="text" id="quick-cliente-nombre" required placeholder="Ej. Juan P├®rez">
                    </div>
                    <div class="form-group">
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            ...
            mostrarNotificacion('Cliente creado con ├®xito', 'success');
            cerrarModal();
            if (callback) {
...
document.body.addEventListener('change', async (e) => {
    if (e.target.id === 'selector-negocio') {
        ...
        if (nuevoNegocioId) {
            ...
            // Re-fetch permisos si es un rol con permisos din├ímicos (Vendedor)
            const user = getCurrentUser();
...
window.addEventListener('popstate', (e) => {
    if (estaActualizandoAuth) return; // Ô£¿ No navegar si estamos re-autenticando
...
// --- ­ƒøí´©Å­ƒøí´©Å RESTAURACI├ôN DE SESI├ôN GLOBAL (AQU├ì ES DONDE SUCEDE LA MAGIA) ­ƒøí´©Å­ƒøí´©Å ---
// Esta funci├│n se ejecuta YA, antes de que nada m├ís pase.
const restaurarSesionGlobal = () => {
    ...
    if (tipo && tipo !== 'null' && tipo !== 'undefined') {
        console.log(`ÔÖ╗´©Å [Global] Restaurando sesi├│n al inicio: ${tipo}`);
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register(`/service-worker.js${v}`).then(reg => {
            // Ô£¿ Escuchar si hay una actualizaci├│n de SW
            reg.onupdatefound = () => {
                const newWorker = reg.installing;
                newWorker.onstatechange = () => {
                    if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                        // Nuevo SW instalado y listo. Recargar.
                        console.log("­ƒÜÇ Nueva versi├│n detectada. Recargando...");
                        if (window.mostrarNotificacion) {
